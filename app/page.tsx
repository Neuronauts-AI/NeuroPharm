...
export default function Home() {
  ...
  const [viewMode, setViewMode] = useState<'default' | 'anamnesis'>('default');
  ...
  const handleAnamnesisAnalysis = async (file: File, medicines: Medicine[]) => {
    ...
    try {
      ...
      // Construct temp patient from extracted info
      const extracted = result.extracted_patient_info || {};
      const currentMeds = (extracted.current_medications || []).map((m: any, idx: number) => ({
        id: `curr-${idx}`,
        name: m.name,
        dosage: m.dosage
      }));
      ...
      const tempPatient: Patient = {
        id: 'temp-anamnesis',
        name: 'Anemnez Analizi',
        age: extracted.age || 45,
        gender: extracted.gender || 'male', // Default
      ...
      // setViewMode('default'); // Keep in anamnesis mode
  ...
  const renderContent = () => {
    if (viewMode === 'anamnesis' || selectedPatient?.id === 'temp-anamnesis') {
      return (
        <AnamnesisPanel
